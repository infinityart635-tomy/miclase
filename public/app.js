...
const NATIVE_PDF_REQUEST_TIMEOUT_MS = 45000;
...
function formatPdfDebugValue(value) {
...
function logPdfDebug(step, details = {}, options = {}) {
  const entries = Object.entries(details)
    .filter(([, value]) => value !== '' && typeof value !== 'undefined')
    .map(([key, value]) => `${key}=${formatPdfDebugValue(value)}`);
  const summary = entries.length ? `${step} | ${entries.join(' | ')}` : step;
  try {
    console.debug('[MiClase PDF]', summary, details);
  } catch (_) {
    // Ignore console issues.
  }
  if (options.visible) {
    setNotice(`PDF debug: ${summary}`);
  }
  return summary;
}
...
function clearPendingNativePdfDownload(itemOrKey) {
  const key = typeof itemOrKey === 'string' ? itemOrKey : getMaterialTransferKey(itemOrKey);
  if (!key) return;
  const pollerId = nativePdfPendingPollers.get(key);
  if (pollerId) {
    window.clearInterval(pollerId);
  }
  nativePdfPendingPollers.delete(key);
  const timerId = nativePdfPendingDownloads.get(key);
  if (timerId) {
...
function isNativePdfSimulationActive(item) {
  const key = getMaterialTransferKey(item);
  return Boolean(key) && nativePdfDownloadSimulations.has(key);
}
...
function searchAndOpenNativePdf(item) {
  if (!hasNativePdfBridge()) return;
  const key = getMaterialTransferKey(item);
  if (!key) return;
  if (nativePdfOpenSearches.has(key)) {
    logPdfDebug('open-search-already-running', {
      key,
  ...
  const finishSearch = () => {
    if (finished) return;
    finished = true;
    const timerId = nativePdfOpenSearches.get(key);
    if (timerId) {
      window.clearInterval(timerId);
    }
    nativePdfOpenSearches.delete(key);
  };
  const tryOpen = () => {
    attempts += 1;
    logPdfDebug('open-search-attempt', {
      key,
      attempt: `${attempts}/${maxAttempts}`,
      downloaded: isNativePdfDownloaded(item),
    ...
    if (isNativePdfDownloaded(item)) {
      finishSearch();
      clearPendingNativePdfDownload(item);
      markMaterialAsDownloaded(item);
      if (state.transfer?.key === getNativePdfSimulationTransferKey(item)) {
        clearTransferState();
      }
      render();
      try {
        ...
        logPdfDebug('open-search-dispatched-to-bridge', {
          key,
          fileName: item?.fileName,
          originalName: item?.originalName,
          title: item?.title,
        ...
        logPdfDebug('open-search-bridge-throw', {
          key,
          fileName: item?.fileName,
          originalName: item?.originalName,
          title: item?.title,
    ...
    if (attempts >= maxAttempts) {
      ...
      setNotice('Todavia se esta ubicando el PDF en el dispositivo.');
    }
  };
  logPdfDebug('open-search-start', {
    key,
    fileName: item?.fileName,
    originalName: item?.originalName,
    title: item?.title,
  ...
  tryOpen();
  if (finished) return;
  const timerId = window.setInterval(tryOpen, intervalMs);
  nativePdfOpenSearches.set(key, timerId);
}

function watchPendingNativePdfDownload(item) {
  const key = getMaterialTransferKey(item);
  if (!key || nativePdfPendingPollers.has(key)) return;
  const transferKey = getNativePdfSimulationTransferKey(item);
  const pollerId = window.setInterval(() => {
    if (!isPendingNativePdfDownload(item)) {
      clearPendingNativePdfDownload(key);
      return;
    }
    if (!isNativePdfDownloaded(item)) {
    ...
    clearPendingNativePdfDownload(key);
    markMaterialAsDownloaded(item);
    if (!isNativePdfSimulationActive(item) && (!state.transfer?.key || state.transfer.key === transferKey)) {
      clearTransferState();
    }
    if (!isNativePdfSimulationActive(item)) {
      setNotice('PDF descargado.');
    }
    render();
  }, 1500);
  nativePdfPendingPollers.set(key, pollerId);
}
...
function startNativePdfDownloadSimulation(item) {
  ...
  if (!itemKey) return;
  cancelTransferStateClear();
  stopNativePdfDownloadSimulation(itemKey, { clearOptimistic: true });
...
function hasNativePdfBridge() {
  return /android/i.test(navigator.userAgent || '') && typeof window.AndroidPdfBridge !== 'undefined';
}

function isNativePdfDownloadedByUrl(url) {
  if (!hasNativePdfBridge() || !url) return false;
  try {
    return Boolean(window.AndroidPdfBridge.isPdfDownloaded(url));
  } catch (_) {
    return false;
  }
}

function getDownloadedNativePdfBridgeUrl(item) {
  const downloadUrl = getAbsoluteMaterialDownloadUrl(item);
  if (isNativePdfDownloadedByUrl(downloadUrl)) {
    return downloadUrl;
  }
  const fileUrl = getAbsoluteMaterialFileUrl(item);
  if (isNativePdfDownloadedByUrl(fileUrl)) {
    return fileUrl;
  }
  return '';
}

function getNativePdfBridgeUrl(item) {
  return getDownloadedNativePdfBridgeUrl(item)
    || getAbsoluteMaterialDownloadUrl(item)
    || getAbsoluteMaterialFileUrl(item)
    || '';
}
...
function openNativePdfWithBridge(item) {
  ...
  if (attempt.index >= attempt.candidates.length) {
    ...
    logPdfDebug('bridge-open-fallback-remote', {
      key: itemKey,
      fileUrl,
  ...
  logPdfDebug('bridge-open-call', {
    key: itemKey,
    candidate: candidateName,
    candidateIndex: `${attempt.index}/${attempt.candidates.length}`,
    fileUrl,
    fileName: item?.fileName,
    originalName: item?.originalName,
    title: item?.title,
...
function renderSubjectMaterialCard(career, subject, item) {
  ...
  const isSimulatingPdfDownload = hasNativePdfBridge() && isPdf && isNativePdfSimulationActive(item);
  const isOptimisticPdfReady = hasNativePdfBridge() && isPdf && isNativePdfOptimisticallyReady(item);
  const isPendingPdfDownload = hasNativePdfBridge() && isPdf && isPendingNativePdfDownload(item);
  const pdfActionLabel = (isDownloaded && !isSimulatingPdfDownload) || isOptimisticPdfReady
    ? 'Abrir PDF'
    : ((isSimulatingPdfDownload || isPendingPdfDownload) ? 'Descargando...' : 'Descargar');
...
function handlePdfAction(item) {
  if (hasNativePdfBridge()) {
    ...
    try {
      ...
      logPdfDebug('handle-pdf-download-dispatched', {
        key: getMaterialTransferKey(item),
        fileUrl: getAbsoluteMaterialFileUrl(item),
        downloadUrl: getAbsoluteMaterialDownloadUrl(item),
        fileName: item?.fileName,
      ...
      stopNativePdfDownloadSimulation(item, { clearTransfer: true, clearOptimistic: true });
      clearTransferState();
      logPdfDebug('handle-pdf-action-error', {
        key: getMaterialTransferKey(item),
        fileName: item?.fileName,
        originalName: item?.originalName,
        title: item?.title,
...
function initNativePdfBridge() {
  window.addEventListener('miclase-pdf-refresh', () => {
    logPdfDebug('native-event-refresh', {}, { visible: true });
    syncDownloadedNativePdfs();
    render();
  });
  window.addEventListener('miclase-pdf-progress', (event) => {
    ...
    const transferKey = material ? getNativePdfSimulationTransferKey(material) : '';
    logPdfDebug('native-event-progress', {
      status,
      progress,
      url: detail.url || '',
      message,
    ...
    if (status === 'started' || status === 'downloading') {
      if (material) {
        markPendingNativePdfDownload(material);
        watchPendingNativePdfDownload(material);
      ...
      setTransferState({
        ...
        progress: progress > 0 ? progress : 8,
